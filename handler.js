...
module.exports = handler = async (mek, conn, map) => {
			...
			options.adReply
				? (content.contextInfo = {
						externalAdReply: {
							...
							body: "",
							thumbnail: await conn.getBuffer("https://telegra.ph/file/45335709b228fb81998f0.jpg"),
							sourceUrl: "https://chat.whatsapp.com/GOFfHo7QN855SbunBqNb65",
						},
				  })
				: "";
			if (
  ...
  if(isGroup){
	...
	global.respon = {
	  ...
	  error:{
	    cmd: "*ᴄᴏᴍᴍᴀɴᴅ ᴇʀʀᴏʀ⚠️*",
	    lv: ["*ᴡᴇʀᴇ ɪꜱ ᴛʜᴇ ʟɪɴᴋ😶‍🌫️*"],
	    api: "sᴇʀᴠᴇʀ ʙᴜsʏ:﹚"
	  },
	  group: "ᴛʜɪs ᴄᴏᴍᴍᴀɴɴᴅ ᴏɴʟʏ ғᴏʀ ɢʀᴏᴜᴩ! :﹚",
	  private: "ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ ᴏɴʟʏ ғᴏʀ ᴩʀɪᴠᴀᴛᴇ ᴄʜᴀᴛ/ ᴩᴄ ʙᴏᴛ :﹚..",
	  admin: "ᴀᴅᴍɪɴ ᴏɴʟʏ ᴄᴀɴ ᴜsᴇ ᴛʜɪs!",
	  bAdmin: "ʙᴏᴛ ɪs ɴᴏᴛ ᴀᴅᴍɪɴ!!",
	  owner: "ᴏɴʟʏ ғᴏʀ ᴍʏ ᴏᴡɴᴇʀ !"
	}
	
	// [ Global Error ]
	global.error = (command, e, msg) => {
	  error = "*Command Error⚠️*\n"
	  error += "  × Fitur : " + command + "\n\n"
	  error += "*Error Log*\n"
	  error += String(e)
