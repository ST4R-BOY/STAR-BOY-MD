  ...
  try {
    const start = async() => {
      ...
      const conn = await makeWaSocket({
        auth: state,
        browser: [`STAR BOY MD`, "Safari", "1.0.0"],
      ...
      conn.ev.on("connection.update", async up => {
        ...
        if(connection == "connecting") return await msg.reply("_*Connecting to Star Boy MD Temporary Bot..*_");
        if(connection){
          if (connection != "connecting") console.log("Connecting to Star Boy MD Temporary Bot..")
        }
        console.log(up)
        if(up.qr) await sendFile(msg.from, await qrcode.toDataURL(up.qr,{scale : 8}),"", 'Scan QR This is to be a temporary bot\n\n1. Click the three dots in the top right corner\n2. Link devices Option \n3. Scan QR \nQR Expires in 20 seconds', msg)
