  ...
  async run({msg,conn},{q,args, map}){
   ...
   try {
     switch(command){
       ...
       case "audio":
         if(sender != args[1])return msg.reply("You haven't requested a song, please request it first..")
         await reply(respon.wait)
       ...
       case "video":
         if(sender != args[1])return msg.reply("You haven't requested a video, please request it first..")
         await reply(respon.wait)
